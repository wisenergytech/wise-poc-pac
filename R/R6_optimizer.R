...
# -----------------------------------------------------------------------------
# DualOptimizer — Dual PAC (ASHP+GSHP) coupled dispatch
# -----------------------------------------------------------------------------

#' @title Dual PAC Optimizer
#' @description R6 optimizer for coupled ASHP+GSHP dispatch. Formulates a single
#'   MILP with binary (on/off) and continuous (inverter) variables, ramp smoothing,
#'   and dual COP iterative refinement. Delegates to solve_block_dual() from
#'   R/optimizer_dual.R.
#' @export
DualOptimizer <- R6::R6Class("DualOptimizer",
  ...
  private = list(
    mode_label = "optimizer_dual",
    solve_block = function(block_data, params, t_init, soc_init, prix_terminal_per_deg) {
      solve_block_dual(block_data, params, t_init, soc_init, prix_terminal_per_deg)
    },
    baseline_fallback = function(bd) {
      params <- private$params
      dplyr::tibble(
        sim_pac_on = 0, sim_pac1_on = 0, sim_pac2_load = 0,
        sim_pac1_kwh = 0, sim_pac2_kwh = 0,
