...
# --- 1a. k0001 : PAC (5-min) — all columns ---
message("[BQ] Fetching k0001 (PAC, SELECT *)...")
sql_pac <- sprintf("
  SELECT *
  FROM `%s.%s.k0001`
  WHERE time >= '%s'
...
message(sprintf("  -> %d lignes PAC, colonnes: %s", nrow(df_pac), paste(names(df_pac), collapse = ", ")))

# Write raw BQ dump (all columns, 5-min resolution)
out_raw_pac <- file.path(DATA_DIR, "bq_k0001_raw.csv")
df_pac_export <- df_pac
df_pac_export$time <- format(df_pac_export$time, "%Y-%m-%d %H:%M:%S")
write_csv(df_pac_export, out_raw_pac)
message(sprintf("[RAW] %s : %d lignes, %d colonnes", out_raw_pac, nrow(df_pac), ncol(df_pac)))

# --- 1b. k0001_ores : Compteur ORES (5-min) — all columns ---
message("[BQ] Fetching k0001_ores (compteur, SELECT *)...")
sql_ores <- sprintf("
  SELECT *
  FROM `%s.%s.k0001_ores`
  WHERE time >= '%s'
...
message(sprintf("  -> %d lignes ORES, colonnes: %s", nrow(df_ores), paste(names(df_ores), collapse = ", ")))

# Write raw BQ dump (all columns, 5-min resolution)
out_raw_ores <- file.path(DATA_DIR, "bq_k0001_ores_raw.csv")
df_ores_export <- df_ores
df_ores_export$time <- format(df_ores_export$time, "%Y-%m-%d %H:%M:%S")
write_csv(df_ores_export, out_raw_ores)
message(sprintf("[RAW] %s : %d lignes, %d colonnes", out_raw_ores, nrow(df_ores), ncol(df_ores)))
...
# =============================================================================
# 6. DATA DICTIONARY (YAML) for fused CSVs
# =============================================================================
yaml_content <- sprintf('# Data dictionary — bq_k0001 fused CSVs
# Generated by scripts/fetch_bigquery_pac.R on %s
source:
  project: %s
  dataset: %s
  script: scripts/fetch_bigquery_pac.R
  generated: "%s"
  period: ["%s", "%s"]
  resolution: 15min
  raw_resolution: 5min
...
raw_dumps:
  bq_k0001_raw.csv:
    ...
    query: "SELECT *"
    resolution: 5min
  bq_k0001_ores_raw.csv:
    table: "%s.%s.k0001_ores"
    query: "SELECT *"
    resolution: 5min

columns:
  timestamp:
    bq_tables: [k0001, k0001_ores]
    bq_column: time
    aggregation: floor_date(15min)
    unit: datetime

  pac_kwh:
    bq_table: k0001
    bq_column: Elec_consumption
    aggregation: sum(15min)
    unit: kWh_e/qt

  t_ballon:
    bq_table: k0001
    bq_column: T_tankUp
    aggregation: mean(15min)
    unit: degC
  ...
  offtake_kwh:
    bq_table: k0001_ores
    bq_column: Consumption_index_kWh
    aggregation: delta(last - lag, 15min)
    unit: kWh/qt

  feedin_kwh:
    bq_table: k0001_ores
    bq_column: Injection_index_kWh
    aggregation: delta(last - lag, 15min)
    unit: kWh/qt

  pv_kwh:
    source_elia: "Elia ODS032/ODS087 (region Namur), scaled to %d kWc"
    source_fusion: "FusionSolar 5-min CSVs (3 inverters), sum(15min)"
    unit: kWh/qt
',
  Sys.Date(), BQ_PROJECT, BQ_DATASET, Sys.Date(), DATE_START, DATE_END,
  BQ_PROJECT, BQ_DATASET, BQ_PROJECT, BQ_DATASET, PV_KWC)

writeLines(yaml_content, file.path(DATA_DIR, "bq_k0001_dictionary.yaml"))
message(sprintf("[YAML] %s written", file.path(DATA_DIR, "bq_k0001_dictionary.yaml")))
